...
$(function(){
   ...
   var id=$("[name=parentName]").val();
   alert(id);
    $("[name=parentName2]").find("option").remove();
    $("[name=parentName2]").append("<option value='0'>请选择二级</option>");
   
    $.ajax(
	    {
		    url:"<%=path%>/servlet/ProListfenleiServlet?action=find&id="+id,
		    data:"id="+id,
		    success:function(data){
		    	$.each(eval("("+data+")"),function(i,item){
		    	$("[name=parentName2]").append("<option value="+item.id+">"+item.name+"</option>")
		    	});
		    }
...
<div id="main" class="wrap">
	...
	<div class="main">
		<h2>修改分类</h2>
		<div class="manage"><div class="manage"><div class="manage"> 
			<form action="<%=path %>/servlet/ProListfenleiServlet?action=find" method="post"> 
				<table class="form"> 
					<tbody><tr> 
						<td class="field">父分类：</td> 
						<td> 
							<select name="parentName"> 
								<option value="0" selected="selected">请选择一级</option> 
								 <c:forEach items="${list}" var="item" >
								   <option value="${item.id}" >${item.name }</option>
								</c:forEach>
							</select> 
						    <select name="parentName2"> 
								<option value="0" selected="selected">请选择二级</option>
...
<div id="footer">
	Copyright &copy; 2017 北大青鸟 All Rights Reserved. 京ICP证1000001号
</div>
</body>
</html>
