...
<script type="text/javascript">
   function del(id){
	   ...
	   if(result){
		   ...
		   var path="<%=path %>/servlet/ProListfenjiServlet?action=del&id="+id;
		   location.href=path;
	   }
	   /*else{
		   //code execute here, it says user click the cancel button
		   alert('cancel');
	   }*/
   }
</script>
...
<div id="main" class="wrap">
		...
		<div class="manage">
			<table class="list">
				...
				<%
						...
						for (ProListfenji item : page2.getList()) {
					       if(item.getType()==1){
					%>
					<tr>
					<td class="first w4 c"><%=item.getId() %></td>
					<td><%=item.getName() %></td>
					<td class="w1 c"><a href="<%=path %>/servlet/ProListfenleiServlet?action=showFenji&type=2">修改</a> <a class="Del" href="javascript:del(<%=item.getId() %>)">删除</a></td>
				</tr>
				<%
				}else{
				%>
				<tr>
					<td class="first w4 c"><%=item.getId() %></td>
					<td class="childClass"><%=item.getName() %></td>
					<td class="w1 c"><a href="<%=path %>/servlet/ProListfenleiServlet?action=showFenji&type=2">修改</a> <a class="Del" href="javascript:del(<%=item.getId() %>)">删除</a></td>
				</tr>
				...
				<%-- <tr>
					...
					<td>大类</td>
					<td class="w1 c"><a href="<%=path %>/Backstage/productClass-modify.jsp">修改</a> <a class="manageDel" href="javascript:void(0)">删除</a></td>
				</tr>
				<tr>
					<td class="first w4 c">1</td>
    ...
    <div class="pager">
				<ul class="clearfix">
                    ...
                    <li><a >6</a></li>
                    <li><a href="<%=path %>/servlet/ProListfenjiServlet?action=fenji&pageindex=<%=page2.getPageindex()+1%>">下一页</a></li>
					<li><a >尾页</a></li>
				</ul>
			</div>
</div>
